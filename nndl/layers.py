...
def affine_backward(dout, cache):
  ...
  # ================================================================ #
  dx = (dout@np.transpose(w)).reshape(x.shape)
  dw = np.transpose(x.reshape(x.shape[0], np.prod(x[0].shape))) @ dout
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    # ================================================================ #
      ...
      running_mean = momentum * running_mean + (1 - momentum) * sample_mean
      running_var = momentum * running_var + (1 - momentum) * sample_var
      xhat = (x - sample_mean)/np.sqrt(sample_var**2 + eps)
      out = gamma*xhat + beta
      cache = (xhat, gamma, beta, x, sample_var, sample_mean, eps)
    ...
    xhat = (x - running_mean)/np.sqrt(running_var**2 + eps)
    out = gamma*xhat + beta
    cache = (xhat, gamma, beta, x, sample_var, sample_mean, eps)
...
def batchnorm_backward(dout, cache):
  ...
  dxhat = dout*gamma #(4,5)

  da = (1/np.sqrt(sample_var**2 + epsilon)) * dxhat

  db = (x - sample_mean)*dxhat
  dc = -1/(sample_var**2 + epsilon) *db
  de = (-1/2)*1/((sample_var**2 + epsilon)**(3/2)) * db
  dstd = np.sum(de, axis = 0)
    
  du = -1/np.sqrt(sample_var**2 + epsilon) * np.sum(dxhat, axis = 0) - dstd*(2/M)*np.sum(x-sample_mean, axis= 0)
    
  dx = da + (2*(x- sample_mean)/M)* dstd + (1/M) *du
#   print(((1/M) *du).shape)
